...
files = glob.glob(folder + "/*.[jJ][pP][gG]")
print(files)
...
for i in range(0, len(files), batch_size):
    ...
    yaltai = YALTAiCommand(
        ...
        yolo_model="models/seg_megv.pt",
        verbose=True,
        raise_on_error=True,
        allow_failure=False,
        multiprocess=1,  # GPU Memory // 5gb
        check_content=False
    )
    yaltai.process()
    ...
    kraken = KrakenRecognizerCommand(
        ...
        device="cpu",
        model="models/fondue_gd_megv_v2.mlmodel",
        multiprocess=1,  # GPU Memory // 3gb
